...
if topology==3
    ...
    I = find(i<j);
    i = i(I); j = j(I);
    [tmp,I] = unique(i + 1234567*j); %eliminate the edge shared by two faces
    i = i(I); j = j(I);
    ne = length(i); % number of edges
    s = n+(1:ne);

    A = sparse([i;j],[j;i],[s;s],n,n); % adjacency map which is symmetrical with the value of (vertex number + edge index)

    % first face, generate the vertices of the sub faces  
    ...
    face1 = [   cat(1,face(1,:),v12,v31),...
        ...
        cat(1,v12,v23,v31)   ]; % generate the sub faces
