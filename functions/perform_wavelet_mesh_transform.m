function [f,edge_vector,vring_out]= perform_wavelet_mesh_transform(vertex,face, f,edge_vector, dir, options)
...
if size(f,2)>1
    for i=1:size(f,2)
        [f(:,i),edge_vector(:,i),vring_out] = perform_wavelet_mesh_transform(vertex,face, f(:,i), edge_vector(:,i),dir, options);
    end
    return;
end
...
for j=jlist
    ...
    for k=nj+1:nj1  % for all the fine point
        % retrieve coarse neighbors
        [e,v,g] = compute_butterfly_neighbors(k, nj);
        % do interpolation with butterfly, fi is the position of the vertex
        % generated by butterfly algorithm
        if isempty(v)||isempty(g)
            fi=1/2*sum(f(e));
        else
        fi = 1/2*sum(f(e)) + 1/8*sum(f(v)) - 1/16*sum(f(g));
        end
        edge_vector(k)=f(e(1))-f(e(2));
        if dir==1
            f(k) = f(k) - fi;
        else
            f(k) = f(k) + fi;
